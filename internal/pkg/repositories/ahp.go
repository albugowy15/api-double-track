...
func (r *AHPRepository) SaveAHPAlternative(data models.AHPToAlternatives) error {
	_, err := db.GetDb().Exec("INSERT INTO ahp_to_alternatives (score, ahp_id, alternative_id) VALUES ($1, $2, $3)", data.Score, data.AhpId, data.AlternativeId)
	if err != nil {
		log.Println("db err:", err)
		return err
	}
...
func (r *AHPRepository) SaveAHPAlternatives(data []models.AHPToAlternatives) error {
	_, err := db.GetDb().NamedExec("INSERT INTO ahp_to_alternatives (score, ahp_id, alternative_id) VALUES (:score, :ahp_id, :alternative_id)", data)
	if err != nil {
		log.Println("db err:", err)
		return err
	}
