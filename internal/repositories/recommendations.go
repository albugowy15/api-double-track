...
func GetTOPSISAHPRecommendations(studentId string) ([]models.RecommendationResult, error) {
	...
	err := db.AppDB.Select(
		&recommendations,
		`SELECT ata.id, a.alternative, ata.score, a.description FROM topsis_ahp_to_alternatives ata 
		INNER JOIN alternatives a ON a.id = ata.alternative_id
		INNER JOIN topsis ON topsis.id = ata.topsis_id
		WHERE topsis.student_id = $1
		ORDER BY ata.score DESC`,
...
func GetTOPSISCombinativeRecommendations(studentId string) ([]models.RecommendationResult, error) {
	...
	err := db.AppDB.Select(
		&recommendations,
		`SELECT ata.id, a.alternative, ata.score, a.description FROM topsis_combinative_to_alternatives ata
		INNER JOIN alternatives a ON a.id = ata.alternative_id
		INNER JOIN topsis ON topsis.id = ata.topsis_id
		WHERE topsis.student_id = $1
		ORDER BY ata.score DESC`,
