...
func CalculateTopsis(r *http.Request, body []models.SubmitAnswerRequest, tx *sqlx.Tx, criteriaWeight ahp.CriteriaWeight) error {
...
func CalculateTopsisAHP(r *http.Request, body []models.SubmitAnswerRequest, tx *sqlx.Tx, criteriaWeight ahp.CriteriaWeight) error {
	var arr_sum [topsis.TotalCriteria]float32
	for _, answer := range body {
		is_interest_question := answer.Number >= 1 && answer.Number <= 7
		is_facilities_question := answer.Number >= 8 && answer.Number <= 14
	...
	_, ahp := weightmethods.AhpWeight(r, body, criteriaWeight)
	log.Println("criteriaWeight Interest AHP : ", ahp.Interest)
	log.Println("criteriaWeight Salary AHP : ", ahp.Salary)
	log.Println("criteriaWeight Facilities AHP : ", ahp.Supporting_Facilities)
	log.Println("criteriaWeight Entrepreneur Opportunities AHP : ", ahp.Entrepreneur_Opportunities)
	log.Println("criteriaWeight Total Open Jobs AHP: ", ahp.Total_open_jobs)

	for idx, val := range interest_norm {
		interest_norm[idx] = val * ahp.Interest
	}

	for idx, val := range facilities_norm {
		facilities_norm[idx] = val * ahp.Supporting_Facilities
	}

	for idx, val := range TotalOpenJobs_norm {
		TotalOpenJobs_norm[idx] = val * ahp.Total_open_jobs
	}

	for idx, val := range EntrepreneurshipOpportunity_norm {
		EntrepreneurshipOpportunity_norm[idx] = val * ahp.Entrepreneur_Opportunities
	}

	for idx, val := range Salaries_norm {
		Salaries_norm[idx] = val * ahp.Salary
	}
	...
	*/
	...
	err = repositories.SaveTOPSISAHPAlternativeTx(data, tx)
	if err != nil {
		return err
	}
...
func CalculateTopsisCombinative(r *http.Request, body []models.SubmitAnswerRequest, tx *sqlx.Tx, criteriaWeight ahp.CriteriaWeight) error {
	var arr_sum [topsis.TotalCriteria]float32
	for _, answer := range body {
		is_interest_question := answer.Number >= 1 && answer.Number <= 7
		is_facilities_question := answer.Number >= 8 && answer.Number <= 14
	...
	_, combinative := weightmethods.CalculateCombinative(r, body, criteriaWeight)
	log.Println("criteriaWeight Interest Combinative : ", combinative.Interest)
	log.Println("criteriaWeight Salary Combinative : ", combinative.Salary)
	log.Println("criteriaWeight Facilities Combinative : ", combinative.Supporting_Facilities)
	log.Println("criteriaWeight Entrepreneur Opportunities Combinative : ", combinative.Entrepreneur_Opportunities)
	log.Println("criteriaWeight Total Open Jobs Combinative : ", combinative.Total_open_jobs)

	for idx, val := range interest_norm {
		interest_norm[idx] = val * combinative.Interest
	}

	for idx, val := range facilities_norm {
		facilities_norm[idx] = val * combinative.Supporting_Facilities
	}

	for idx, val := range TotalOpenJobs_norm {
		TotalOpenJobs_norm[idx] = val * combinative.Total_open_jobs
	}

	for idx, val := range EntrepreneurshipOpportunity_norm {
		EntrepreneurshipOpportunity_norm[idx] = val * combinative.Entrepreneur_Opportunities
	}

	for idx, val := range Salaries_norm {
		Salaries_norm[idx] = val * combinative.Salary
	}
	...
	*/
	...
	err = repositories.SaveTOPSISCombinativeAlternativeTx(data, tx)
	if err != nil {
		return err
	}
