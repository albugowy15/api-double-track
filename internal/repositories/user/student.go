...
func (r *StudentRepository) GetStudentByUsername(username string) (user.Student, error) {
	...
	err := db.AppDB.Get(&student, "SELECT id, username, email, password, phone_number, fullname, nisn  FROM students WHERE username = $1", username)
	return student, err
}

func (r *StudentRepository) GetStudentsBySchool(schoolId string) ([]user.Student, error) {
	students := []user.Student{}
	err := db.AppDB.Select(&students, "SELECT id, username, email, phone_number, fullname, nisn FROM students WHERE school_id = $1", schoolId)
	return students, err
}

func (r *StudentRepository) GetStudentById(studentId string) (user.Student, error) {
	student := user.Student{}
	err := db.AppDB.Get(&student, "SELECT id, username, email, phone_number, fullname, nisn FROM students WHERE id = $1", studentId)
	return student, err
}

func (r *StudentRepository) GetStudentBySchoolId(schoolId string, studentId string) (user.Student, error) {
	student := user.Student{}
	err := db.AppDB.Get(&student, "SELECT id, username, email, phone_number, fullname, nisn FROM students WHERE school_id = $1 AND id = $2", schoolId, studentId)
	return student, err
}

func (r *StudentRepository) AddStudent(schoolId string, data user.Student) error {
	_, err := db.AppDB.Exec("INSERT INTO students (username, password, fullname, nisn, school_id) VALUES ($1, $2, $3, $4, $5)", data.Username, data.Password, data.Fullname, data.Nisn, schoolId)
	return err
}
